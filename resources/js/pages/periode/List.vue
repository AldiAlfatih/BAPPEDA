...
const breadcrumbs: BreadcrumbItem[] = [
  ...
  { title: 'Daftar Periode', href: route('periode.index') },
];
...
<template>
  ...
  <AppLayout :breadcrumbs="breadcrumbs">
    <div class="p-4 bg-white rounded-lg shadow">
      ...
      <div v-if="periodes.length === 0" class="p-8 text-center text-gray-500">
        <div class="mb-2 text-5xl">📅</div>
        <h3 class="mb-2 text-xl font-medium">Tidak Ada Periode</h3>
        <p>Belum ada periode yang tersedia dalam sistem.</p>
      </div>
      ...
      <div v-else class="overflow-x-auto">
        <table class="w-full border-collapse">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="(p, index) in periodes" :key="p.id" class="hover:bg-gray-50">
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ index + 1 }}</td>
              <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ p.tahap.tahap }}</td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ p.tahun.tahun }}</td>
