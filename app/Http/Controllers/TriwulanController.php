...
class TriwulanController extends Controller
{
    ...
    public function showDetail(int $tid, string $id, string $taskId, int $tahun = null)
    {
        ...
        // PERBAIKAN: Determine urusan ID based on task context to ensure consistency
        // Similar fix to RencanaAwalController - preserve original context
        $urusanId = $tugas->kodeNomenklatur->details->first()->id_urusan ?? null;
        
        // DEBUG: Log current task details for analysis
        \Log::info('TriwulanController.showDetail: Task analysis for consistency', [
            'task_id' => $tugas->id,
            'task_id_parameter' => $taskId,
            'kode_nomenklatur_id' => $tugas->kodeNomenklatur->id,
            ...
            'extracted_urusan_id' => $urusanId,
        ]);

        // PERBAIKAN: If task represents a specific urusan level, derive urusan from task itself
        // This prevents data inconsistency where task ID 7 should show urusan 7 data
        if ($tugas->kodeNomenklatur->jenis_nomenklatur <= 1) {
            // For high-level tasks (urusan or bidang urusan level), extract urusan from nomor_kode
            $taskCode = $tugas->kodeNomenklatur->nomor_kode ?? '';
            
            // Extract urusan number from code (e.g., "7.01" -> "7", "8" -> "8")
            $urusanNumber = explode('.', $taskCode)[0];
            
            if (is_numeric($urusanNumber)) {
                // Find urusan by nomor_kode instead of relying on potentially inconsistent id_urusan
                $consistentUrusan = KodeNomenklatur::where('jenis_nomenklatur', 0)
                    ->where('nomor_kode', $urusanNumber)
                    ->first();
                    
                if ($consistentUrusan) {
                    $urusanId = $consistentUrusan->id;
                    \Log::info('TriwulanController.showDetail: Using consistent urusan derived from task code', [
                        'task_code' => $taskCode,
                        'urusan_number' => $urusanNumber,
                        'consistent_urusan_id' => $urusanId,
                        'urusan_nomenklatur' => $consistentUrusan->nomenklatur,
                    ]);
                }
            }
        }

        // Fallback: jika id_urusan pada tugas tidak tersedia, ambil dari salah satu tugas SKPD yang memiliki id_urusan
        if (is_null($urusanId)) {
            \Log::warning('TriwulanController.showDetail: id_urusan is null for selected task, applying fallback', [
                'task_id' => $tugas->id,
                'kode_nomenklatur_id' => $tugas->kodeNomenklatur->id,
                'jenis_nomenklatur' => $tugas->kodeNomenklatur->jenis_nomenklatur,
            ]);

            $fallback = $skpdTugas->first(function ($item) {
            ...
            if ($fallback) {
                $urusanId = $fallback->kodeNomenklatur->details->first()->id_urusan;
                \Log::info('TriwulanController.showDetail: Using fallback urusanId from another task', [
        ...
        // DEBUG: Log final urusan data being used
        \Log::info('TriwulanController.showDetail: Final urusan data for filtering', [
            'final_urusan_id' => $urusanId,
            'urusan_found' => $urusan ? true : false,
            'urusan_nomor_kode' => $urusan ? $urusan->nomor_kode : 'NULL',
            'urusan_nomenklatur' => $urusan ? $urusan->nomenklatur : 'NULL',
        ]);
        ...
        $bidangurusanTugas = $skpdTugas->filter(function($item) use ($urusanId) {
            if ($item->kodeNomenklatur->jenis_nomenklatur != 1) return false;
            $detail = $item->kodeNomenklatur->details->first();
            if (!$detail) return false;
            
            $isMatch = is_null($urusanId) ? true : ($detail->id_urusan == $urusanId);
            
            // DEBUG: Log filtering process for bidang urusan
            if ($item->kodeNomenklatur->jenis_nomenklatur == 1) {
                \Log::info('TriwulanController.showDetail: Filtering bidang urusan', [
                    'item_id' => $item->id,
                    'item_nomor_kode' => $item->kodeNomenklatur->nomor_kode,
                    'item_nomenklatur' => $item->kodeNomenklatur->nomenklatur,
                    'item_detail_id_urusan' => $detail->id_urusan ?? 'NULL',
                    'filter_urusan_id' => $urusanId,
                    'is_match' => $isMatch,
                ]);
            }
        ...
        \Log::info('TriwulanController.showDetail: Filtered bidang urusan results', [
            'total_bidang_urusan_found' => $bidangurusanTugas->count(),
            'bidang_urusan_codes' => $bidangurusanTugas->map(function($item) {
                return $item->kodeNomenklatur->nomor_kode . ' - ' . $item->kodeNomenklatur->nomenklatur;
            })->toArray(),
        ]);

        $programTugas = $skpdTugas->filter(function($item) use ($urusanId) {
            if ($item->kodeNomenklatur->jenis_nomenklatur != 2) return false;
            $detail = $item->kodeNomenklatur->details->first();
            if (!$detail) return false;
            // Program detail biasanya memiliki id_urusan; fallback: jika urusanId null, ambil semua program
            return is_null($urusanId) ? true : ($detail->id_urusan == $urusanId);
        })->values();

        $kegiatanTugas = $skpdTugas->filter(function($item) use ($urusanId) {
            if ($item->kodeNomenklatur->jenis_nomenklatur != 3) return false;
            $detail = $item->kodeNomenklatur->details->first();
            if (!$detail) return false;
            return is_null($urusanId) ? true : ($detail->id_urusan == $urusanId);
        })->values();
        ...
        $subkegiatanTugas = $skpdTugas->filter(function($item) use ($urusanId) {
            if ($item->kodeNomenklatur->jenis_nomenklatur != 4) return false;
            $detail = $item->kodeNomenklatur->details->first();
            if (!$detail) return false;
            // Jika urusanId tidak diketahui, tampilkan semua subkegiatan aktif agar tidak kosong
            return is_null($urusanId) ? true : ($detail->id_urusan == $urusanId);
        })->values();
        ...
        $monitoring = \App\Models\Monitoring::where('skpd_tugas_id', $tugas->id)
            ->where('tahun', $tahun)
            ->with(['anggaran.target.periode.tahap', 'anggaran.realisasi.periode.tahap'])
            ->get();
        ...
        $allMonitoring = \App\Models\Monitoring::whereHas('tugas', function($query) use ($tugas) {
            ...
            ->where('tahun', $tahun)
            ->with(['anggaran.target.periode.tahap', 'anggaran.realisasi.periode.tahap', 'anggaran.sumberAnggaran', 'anggaran.pagu'])
            ->get();
            ...
            foreach ($monitoringItem->anggaran as $anggaran) {
                ...
                foreach ($anggaran->target as $target) {
                    $monitoringTargets[] = [
                        ...
                        'periode_tahap' => $target->periode && $target->periode->tahap ? $target->periode->tahap->tahap : null,
                ...
                foreach ($anggaran->realisasi as $realisasi) {
                    $monitoringRealisasi[] = [
                        ...
                        'periode_tahap' => $realisasi->periode && $realisasi->periode->tahap ? $realisasi->periode->tahap->tahap : null,
                        'monitoring_id' => $monitoringItem->id,
                        'monitoring_anggaran_id' => $anggaran->id,
                        'deskripsi' => $monitoringItem->deskripsi,
                        'nama_pptk' => $monitoringItem->nama_pptk,
        ...
        \Log::info("TriwulanController.showDetail: Prepared data", [
            ...
            'urusan_id' => $urusanId,
            'counts' => [
                'bidang_urusan' => $bidangurusanTugas->count(),
                'program' => $programTugas->count(),
                'kegiatan' => $kegiatanTugas->count(),
                'subkegiatan' => $subkegiatanTugas->count(),
            ],
            'skpd_data' => [
    ...
    // Removed unused resource stub methods (create, store, edit) since Triwulan uses explicit routes
