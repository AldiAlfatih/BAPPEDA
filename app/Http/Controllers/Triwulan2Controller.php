...
class Triwulan2Controller extends Controller
{
    public function index()
    {
        ...
        if ($user->hasRole('perangkat_daerah')) {
            return redirect()->route('triwulan2.show', $user->id);
        }
     ...
     public function show(string $id)
   {
       ...
       return Inertia::render('Triwulan2/Show', [
           'user' => $user,
           'skpdTugas' => $skpdTugas,
   ...
   public function showDetail($id)
    {
        ...
        // Get all monitoring records for these tasks with filtered monitoringTarget by periode_id = 3 (Triwulan 2)
        $monitorings = \App\Models\Monitoring::whereIn('skpd_tugas_id', $taskIds)
            ->with(['monitoringAnggaran' => function($query) {
                $query->with(['monitoringTarget' => function($query) {
                    // Filter monitoringTarget untuk hanya mengambil dengan periode_id = 3 (Triwulan 2)
                    $query->where('periode_id', 3);
                    $query->with('periode');
                }, 'monitoringRealisasi' => function($query) {
                    // Filter monitoringRealisasi untuk hanya mengambil dengan periode_id = 3 (Triwulan 2)
                    $query->where('periode_id', 3);
                    $query->with('periode');
                }]);
            }])
        ...
        // Process monitoring data - khusus periode_id = 3 (Triwulan 2)
        foreach ($monitorings as $monitoring) {
            ...
            foreach ($monitoring->monitoringAnggaran as $anggaran) {
                // Process monitoring targets - periode_id = 3 only (Triwulan 2)
                if (!$anggaran->monitoringTarget->isEmpty()) {
                    foreach ($anggaran->monitoringTarget as $target) {
                        // Double check - hanya ambil dengan periode_id = 3
                        if ($target->periode_id != 3) {
                            continue;
                        }
        ...
        \Log::info('Total monitoring targets (periode_id = 3): ' . count($monitoringTargets));
        \Log::info('Total monitoring realisasi (periode_id = 3): ' . count($monitoringRealisasi));
        
        // We don't need urusan data, only bidang urusan
        return Inertia::render('Triwulan2/Detail', [
            'tugas' => $tugas,
    ...
    public function saveRealisasi(Request $request)
    {
        ...
        // Check if the Triwulan 2 period is open
        $periodeTriwulan2 = Periode::whereHas('tahap', function($query) {
            $query->where('tahap', 'like', '%Triwulan 2%');
        })->where('status', 1)->first();
        
        // If Triwulan 2 period is not active, return error
        if (!$periodeTriwulan2) {
            return response()->json([
                'success' => false,
                'message' => 'Periode Triwulan 2 belum dibuka. Data tidak dapat disimpan.'
            ], 403);
        }
        ...
        $monitoring = Monitoring::firstOrCreate(
            [
                ...
                'deskripsi' => 'Realisasi Triwulan 2' // Use specific deskripsi for Triwulan 2
            ],
            [
                'tahun' => date('Y'),
        ...
        if ($request->has('keterangan') || $request->has('nama_pptk')) {
            // Keep the deskripsi as 'Realisasi Triwulan 2' to prevent overwriting
        ...
        $realisasi = MonitoringRealisasi::firstOrCreate(
            [
                'monitoring_anggaran_id' => $anggaran->id,
                'periode_id' => $periodeTriwulan2->id
            ],
            [
