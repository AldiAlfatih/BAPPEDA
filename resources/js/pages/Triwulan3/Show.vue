...
function ShowTugas(tugasId: number) {
    // Get current user ID from the URL params - since we're at /triwulan/3/{id}
    const currentUserId = window.location.pathname.split('/')[3];
    router.visit(route('triwulan.detail', { tid: 3, id: currentUserId, taskId: tugasId }));
}
