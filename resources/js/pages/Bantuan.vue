...
import { Plus, MessageSquareText } from 'lucide-vue-next'  // perbaikan import
          ...
          <table class="min-w-full divide-y divide-gray-200 bg-white rounded shadow">
            ...
            <tbody class="divide-y divide-gray-200">
              <tr v-for="(b, index) in filteredBantuans" :key="b.id">
                <td v-if="columns.no" class="px-4 py-2 text-center text-sm text-gray-500">{{ index + 1 }}</td>
                <td v-if="columns.judul" class="px-4 py-2 text-justify text-sm text-gray-500">{{ b.judul }}</td>
                <td v-if="columns.tanggal_dibuat" class="px-4 py-2 text-center text-sm text-gray-500">{{ formatDate(b.created_at) }}</td>
                <td v-if="columns.status" class="px-4 py-2 text-center text-sm text-gray-500">
                  {{ b.status === 0 ? 'Diterima' : b.status === 1 ? 'Diproses' : 'Selesai' }}
                </td>
                <td v-if="columns.aksi" class="px-4 py-2 text-center text-sm text-gray-700">
                  <div class="flex justify-center space-x-2">
                    <Link
                      :href="route('bantuan.chat', b.id)"
                      @click.prevent="updateStatusToDiproses(b.id)"
                      class="px-2 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 flex items-center gap-1 w-max"
                    >
                      <MessageSquareText class="w-4 h-4 mr-1" />
                      Chat
                    </Link>
