   ...
   public function showDetail($id)
    {
        ...
        // Get all monitoring records for these tasks with filtered monitoringTarget by periode_id = 4 (Triwulan 3)
        $monitorings = \App\Models\Monitoring::whereIn('skpd_tugas_id', $taskIds)
            ->with(['monitoringAnggaran' => function($query) {
                $query->with(['monitoringTarget' => function($query) {
                    // Filter monitoringTarget untuk hanya mengambil dengan periode_id = 4 (Triwulan 3)
                    $query->where('periode_id', 4);
                    $query->with('periode');
                }, 'monitoringRealisasi' => function($query) {
                    // Filter monitoringRealisasi untuk hanya mengambil dengan periode_id = 4 (Triwulan 3)
                    $query->where('periode_id', 4);
                    $query->with('periode');
                }]);
            }])
        ...
        // Process monitoring data - khusus periode_id = 4 (Triwulan 3)
        foreach ($monitorings as $monitoring) {
            ...
            foreach ($monitoring->monitoringAnggaran as $anggaran) {
                // Process monitoring targets - periode_id = 4 only (Triwulan 3)
                if (!$anggaran->monitoringTarget->isEmpty()) {
                    foreach ($anggaran->monitoringTarget as $target) {
                        // Double check - hanya ambil dengan periode_id = 4
                        if ($target->periode_id != 4) {
                            continue;
                        }
        ...
        \Log::info('Total monitoring targets (periode_id = 4): ' . count($monitoringTargets));
        \Log::info('Total monitoring realisasi (periode_id = 4): ' . count($monitoringRealisasi));
    ...
    public function saveRealisasi(Request $request)
    {
        ...
        // Check if the Triwulan 3 period is open
        $periodeTriwulan3 = Periode::whereHas('tahap', function($query) {
            $query->where('tahap', 'like', '%Triwulan 3%');
        })->where('status', 1)->first();
        
        // If Triwulan 3 period is not active, return error
        if (!$periodeTriwulan3) {
            return response()->json([
                'success' => false,
                'message' => 'Periode Triwulan 3 belum dibuka. Data tidak dapat disimpan.'
            ], 403);
        }
        ...
        $monitoring = Monitoring::firstOrCreate(
            [
                ...
                'deskripsi' => 'Realisasi Triwulan 3' // Use specific deskripsi for Triwulan 3
            ],
            [
                'tahun' => date('Y'),
        ...
        if ($request->has('keterangan') || $request->has('nama_pptk')) {
            // Keep the deskripsi as 'Realisasi Triwulan 3' to prevent overwriting
        ...
        $realisasi = MonitoringRealisasi::firstOrCreate(
            [
                'monitoring_anggaran_id' => $anggaran->id,
                'periode_id' => $periodeTriwulan3->id
            ],
            [
