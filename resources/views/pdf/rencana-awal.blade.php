    ...
    <table class="table">
        ...
        <tbody>
            ...
            @forelse($bidangurusanTugas ?? [] as $bidangUrusan)
                @php
                    // Validate bidang urusan data
                    if (!$bidangUrusan || !$bidangUrusan->kodeNomenklatur) {
                        continue;
                    }
                    
                    // Get programs under this bidang urusan
                    $programsInBidang = ($programTugas ?? collect())->filter(function ($program) use ($bidangUrusan) {
                        if (!$program || !$program->kodeNomenklatur || !$bidangUrusan->kodeNomenklatur) {
                            return false;
                        }
                    ...
                    foreach ($programsInBidang as $program) {
                        if (!$program || !$program->kodeNomenklatur) {
                            continue;
                        }
                        
                        $kegiatanInProgram = ($kegiatanTugas ?? collect())->filter(function ($kegiatan) use ($program) {
                            if (!$kegiatan || !$kegiatan->kodeNomenklatur || !$program->kodeNomenklatur) {
                                return false;
                            }
                            return str_starts_with(
                        ...
                        foreach ($kegiatanInProgram as $kegiatan) {
                            if (!$kegiatan || !$kegiatan->kodeNomenklatur) {
                                continue;
                            }
                            
                            $subKegiatanInKegiatan = ($subkegiatanTugas ?? collect())->filter(function ($subkegiatan) use ($kegiatan) {
                                if (!$subkegiatan || !$subkegiatan->kodeNomenklatur || !$kegiatan->kodeNomenklatur) {
                                    return false;
                                }
                                return str_starts_with(
                            ...
                            foreach ($subKegiatanInKegiatan as $subkegiatan) {
                                if (!$subkegiatan || !isset($subkegiatan->id)) {
                                    continue;
                                }
                                ...
                                foreach ($targetsBySumber as $targetsForSumber) {
                                    ...
                                    if (!$firstTarget || !is_array($firstTarget)) {
                                        continue;
                                    }
                                    ...
                                    // Safely get target data with validation
                                    $target1 = $targetsByPeriode->get(2);
                                    $target2 = $targetsByPeriode->get(3);
                                    $target3 = $targetsByPeriode->get(4);
                                    $target4 = $targetsByPeriode->get(5);
                                    
                                    $urusanTarget1 += (is_array($target1) ? ($target1['keuangan'] ?? 0) : 0);
                                    $urusanTarget2 += (is_array($target2) ? ($target2['keuangan'] ?? 0) : 0);
                                    $urusanTarget3 += (is_array($target3) ? ($target3['keuangan'] ?? 0) : 0);
                                    $urusanTarget4 += (is_array($target4) ? ($target4['keuangan'] ?? 0) : 0);

                                    // Calculate kinerja fisik for averaging
                                    $urusanKinerjaFisik1 += (is_array($target1) ? ($target1['kinerja_fisik'] ?? 0) : 0);
                                    $urusanKinerjaFisik2 += (is_array($target2) ? ($target2['kinerja_fisik'] ?? 0) : 0);
                                    $urusanKinerjaFisik3 += (is_array($target3) ? ($target3['kinerja_fisik'] ?? 0) : 0);
                                    $urusanKinerjaFisik4 += (is_array($target4) ? ($target4['kinerja_fisik'] ?? 0) : 0);
                                    $countSubKegiatan++;
                                }
                            }
                        }
                ...
                @php
                    ...
                    foreach (($subkegiatanTugas ?? []) as $subkegiatan) {
                        ...
                        foreach ($targetsBySumber as $targetsForSumber) {
                            ...
                            // Sum targets by periode with validation
                            $targetsByPeriode = $targetsForSumber->keyBy('periode_id');
                            ...
                            $totalTarget1 += (is_array($target1) ? ($target1['keuangan'] ?? 0) : 0);
                            $totalTarget2 += (is_array($target2) ? ($target2['keuangan'] ?? 0) : 0);
                            $totalTarget3 += (is_array($target3) ? ($target3['keuangan'] ?? 0) : 0);
                            $totalTarget4 += (is_array($target4) ? ($target4['keuangan'] ?? 0) : 0);
                        }
                    }
                @endphp
