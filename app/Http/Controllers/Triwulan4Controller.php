...
class Triwulan4Controller extends Controller
{
    public function index()
    {
        ...
        if ($user->hasRole('perangkat_daerah')) {
            return redirect()->route('triwulan4.show', $user->id);
        }
   ...
   public function showDetail($id)
    {
        ...
        // Get all monitoring records for these tasks with filtered monitoringTarget by periode_id = 5 (Triwulan 4)
        $monitorings = \App\Models\Monitoring::whereIn('skpd_tugas_id', $taskIds)
            ->with(['monitoringAnggaran' => function($query) {
                $query->with(['monitoringTarget' => function($query) {
                    // Filter monitoringTarget untuk hanya mengambil dengan periode_id = 5 (Triwulan 4)
                    $query->where('periode_id', 5);
                    $query->with('periode');
                }, 'monitoringRealisasi' => function($query) {
                    // Filter monitoringRealisasi untuk hanya mengambil dengan periode_id = 5 (Triwulan 4)
                    $query->where('periode_id', 5);
                    $query->with('periode');
                }]);
            }])
        ...
        // Process monitoring data - khusus periode_id = 5 (Triwulan 4)
        foreach ($monitorings as $monitoring) {
            ...
            foreach ($monitoring->monitoringAnggaran as $anggaran) {
                // Process monitoring targets - periode_id = 5 only (Triwulan 4)
                if (!$anggaran->monitoringTarget->isEmpty()) {
                    foreach ($anggaran->monitoringTarget as $target) {
                        // Double check - hanya ambil dengan periode_id = 5
                        if ($target->periode_id != 5) {
                            continue;
                        }
        ...
        \Log::info('Total monitoring targets (periode_id = 5): ' . count($monitoringTargets));
        \Log::info('Total monitoring realisasi (periode_id = 5): ' . count($monitoringRealisasi));
    ...
    public function saveRealisasi(Request $request)
    {
        ...
        // Check if the Triwulan 4 period is open
        $periodeTriwulan4 = Periode::whereHas('tahap', function($query) {
            $query->where('tahap', 'like', '%Triwulan 4%');
        })->where('status', 1)->first();
        
        // If Triwulan 4 period is not active, return error
        if (!$periodeTriwulan4) {
            return response()->json([
                'success' => false,
                'message' => 'Periode Triwulan 4 belum dibuka. Data tidak dapat disimpan.'
            ], 403);
        }
        ...
        $monitoring = Monitoring::firstOrCreate(
            [
                ...
                'deskripsi' => 'Realisasi Triwulan 4' // Use specific deskripsi for Triwulan 4
            ],
            [
                'tahun' => date('Y'),
        ...
        if ($request->has('keterangan') || $request->has('nama_pptk')) {
            // Keep the deskripsi as 'Realisasi Triwulan 4' to prevent overwriting
        ...
        $realisasi = MonitoringRealisasi::firstOrCreate(
            [
                'monitoring_anggaran_id' => $anggaran->id,
                'periode_id' => $periodeTriwulan4->id
            ],
            [
