...
<body>
    ...
    <div style="background-color: #fff3cd; border: 1px solid #ffeaa7; padding: 10px; margin: 10px 0; font-size: 10px;">
        <strong>DEBUG INFO:</strong><br>
        Current Urusan: {{ $currentUrusan ? ($currentUrusan->kodeNomenklatur->nomor_kode . ' - ' . $currentUrusan->kodeNomenklatur->nomenklatur) : 'NONE' }}<br>
        Bidang Urusan Count: {{ $bidangurusanTugas->count() }}<br>
        Program Count: {{ $programTugas->count() }}<br>
        Kegiatan Count: {{ $kegiatanTugas->count() }}<br>
        Sub Kegiatan Count: {{ $subkegiatanTugas->count() }}<br>
        Monitoring Count: {{ $monitoring->count() }}<br>
        Targets Count: {{ count($monitoringTargets) }}<br>
        Realisasi Count: {{ count($monitoringRealisasi) }}<br>
        
        @if($bidangurusanTugas->count() > 1)
            <strong style="color: red;">WARNING: Multiple Urusan Found!</strong><br>
            @foreach($bidangurusanTugas as $urusan)
                - {{ $urusan->kodeNomenklatur->nomor_kode ?? 'N/A' }} : {{ $urusan->kodeNomenklatur->nomenklatur ?? 'N/A' }}<br>
            @endforeach
        @endif
    </div>
        ...
        <tbody>
            ...
            @forelse(($currentUrusan ? collect([$currentUrusan]) : $bidangurusanTugas) as $bidangUrusan)
                    ...
                    foreach($programsInBidang as $program) {
                        ...
                        foreach($kegiatanInProgram as $kegiatan) {
                            ...
                            foreach($subKegiatanInKegiatan as $subkegiatan) {
                                $targetForTask = collect($monitoringTargets)->where('task_id', $subkegiatan->id)->where('periode_id', $periode->id);
                                $realisasiForTask = collect($monitoringRealisasi)->where('task_id', $subkegiatan->id)->where('periode_id', $periode->id);
                                $targetsBySumber = $targetForTask->groupBy('sumber_anggaran_id');
                                $realisasiBySumber = $realisasiForTask->groupBy('sumber_anggaran_id');
                                $allSumberIds = collect($targetsBySumber->keys())->merge($realisasiBySumber->keys())->unique();
                ...
                <tr style="background-color: #e8f4fd; font-weight: bold;">
                    ...
                    <td class="center"><strong>{{ $urusanTargetKeuangan > 0 ? 'Rp ' . number_format($urusanTargetKeuangan, 0, ',', '.') : '-' }}</strong></td>
                    <td class="center"><strong>{{ number_format($avgUrusanRealisasiFisik, 1) }}%</strong></td>
                    <td class="center"><strong>{{ $urusanRealisasiKeuangan > 0 ? 'Rp ' . number_format($urusanRealisasiKeuangan, 0, ',', '.') : '-' }}</strong></td>
                    <td colspan="2">&nbsp;</td>
                </tr>
                ...
                @foreach($programsInBidang as $program)
                    ...
                    <tr style="background-color: #f0f8ff; font-weight: bold;">
                        ...
                        <td class="center"><strong>{{ $programTargetKeuangan > 0 ? 'Rp ' . number_format($programTargetKeuangan, 0, ',', '.') : '-' }}</strong></td>
                        <td class="center"><strong>{{ number_format($avgProgramRealisasiFisik, 1) }}%</strong></td>
                        <td class="center"><strong>{{ $programRealisasiKeuangan > 0 ? 'Rp ' . number_format($programRealisasiKeuangan, 0, ',', '.') : '-' }}</strong></td>
                        <td colspan="2">&nbsp;</td>
                    </tr>
                    ...
                    @foreach($kegiatanInProgram as $kegiatan)
                        ...
                        <tr style="background-color: #f8fcff; font-weight: bold;">
                            ...
                            <td class="center"><strong>{{ $kegiatanTargetKeuangan > 0 ? 'Rp ' . number_format($kegiatanTargetKeuangan, 0, ',', '.') : '-' }}</strong></td>
                            <td class="center"><strong>{{ number_format($avgKegiatanRealisasiFisik, 1) }}%</strong></td>
                            <td class="center"><strong>{{ $kegiatanRealisasiKeuangan > 0 ? 'Rp ' . number_format($kegiatanRealisasiKeuangan, 0, ',', '.') : '-' }}</strong></td>
                            <td colspan="2">&nbsp;</td>
                        </tr>
                        ...
                        @foreach($subKegiatanInKegiatan as $subkegiatan)
                            ...
                            @foreach($allSumberIds as $sumberAnggaranId)
                                @php
                                    ...
                                    $sumberDana = $targetItem['sumber_anggaran_nama'] ?? $realisasiItem['sumber_anggaran_nama'] ?? '-';
                                @endphp
                                ...
                                <tr>
                                    ...
                                    <td class="center">{{ $targetKeuangan > 0 ? 'Rp ' . number_format($targetKeuangan, 0, ',', '.') : '-' }}</td>
                                    <td class="center">{{ number_format($realisasiFisik, 1) }}%</td>
                                    <td class="center">{{ $realisasiKeuangan > 0 ? 'Rp ' . number_format($realisasiKeuangan, 0, ',', '.') : '-' }}</td>
            ...
            @if($totalPagu > 0)
                ...
                <tr class="total-row">
                    ...
                    <td class="center"><strong>{{ $totalTargetKeuangan > 0 ? 'Rp ' . number_format($totalTargetKeuangan, 0, ',', '.') : '-' }}</strong></td>
                    <td class="center"><strong>{{ number_format($avgRealisasiFisik, 1) }}%</strong></td>
                    <td class="center"><strong>{{ $totalRealisasiKeuangan > 0 ? 'Rp ' . number_format($totalRealisasiKeuangan, 0, ',', '.') : '-' }}</strong></td>
                    <td colspan="2">&nbsp;</td>
                </tr>
            @endif
        </tbody>
    ...
    @if($totalPagu > 0)
        <div class="summary-box">
            <h3 style="margin: 0 0 15px 0; text-align: center;">Ringkasan Kinerja {{ $jenis_laporan }}</h3>
            
            <div class="summary-item">
                <span><strong>Total Pagu Anggaran:</strong></span>
                <span><strong>Rp {{ number_format($totalPagu, 0, ',', '.') }}</strong></span>
            </div>
            
            <div class="summary-item">
                <span>Realisasi Keuangan:</span>
                <span>Rp {{ number_format($totalRealisasiKeuangan, 0, ',', '.') }} ({{ $totalPagu > 0 ? number_format(($totalRealisasiKeuangan / $totalPagu) * 100, 2) : '0.00' }}%)</span>
            </div>
            
            <div class="summary-item">
                <span>Rata-rata Kinerja Fisik:</span>
                <span>{{ number_format($avgRealisasiFisik, 2) }}%</span>
            </div>
            
            <div class="summary-item">
                <span>Sisa Anggaran:</span>
                <span>Rp {{ number_format($totalPagu - $totalRealisasiKeuangan, 0, ',', '.') }}</span>
            </div>
        </div>
    @endif
