...
class RencanaAwalController extends Controller
{
    ...
    public function show($id, Request $request, $tahun = null)
    {
        ...
        // Ambil urusan yang dipilih dari route (prioritas) atau query (fallback)
        $requestedUrusanId = $request->input('urusan_id');
        $routeUrusanId = $request->route('urusanId');
        ...
        // Load basic data menggunakan helper
        ...
        // PERBAIKAN PENTING: Tentukan urusan yang dipilih secara deterministik
        // 1) Jika dikirim dari frontend (query urusan_id) gunakan itu
        // 2) Jika tidak ada, derive dari tugas yang diklik:
        //    - Jika tugas adalah URUSAN (jenis = 0), gunakan ID kode nomenklatur urusan tsb
        //    - Jika tugas adalah selain urusan (jenis >= 1), gunakan details.first.id_urusan
        // 3) Fallback ke urusan pertama yang tersedia untuk SKPD ini
        $availableUrusanIds = $this->getAvailableUrusans($skpdTugas); // array of IDs (non-null)

        if ($routeUrusanId) {
            $urusanId = (int) $routeUrusanId;
        } elseif ($requestedUrusanId) {
            $urusanId = (int) $requestedUrusanId;
        } else {
            $urusanId = null;
            if ($tugas->kodeNomenklatur) {
                if ((int)$tugas->kodeNomenklatur->jenis_nomenklatur === 0) {
                    // Tugas adalah URUSAN → pakai ID kode nomenklatur urusan
                    $urusanId = $tugas->kodeNomenklatur->id;
                } else {
                    // Selain urusan → ambil parent urusan dari details
                    $detail = $tugas->kodeNomenklatur->details->first();
                    if ($detail && $detail->id_urusan) {
                        $urusanId = $detail->id_urusan;
                    }
                }
            }

            if (!$urusanId && !empty($availableUrusanIds)) {
                $urusanId = $availableUrusanIds[0];
            }
        }

        // Canonicalize URL: if urusan dikirim via query dan belum ada di path, redirect ke clean URL
        if ($requestedUrusanId && !$routeUrusanId && $urusanId) {
            $periodeId = $request->input('periode_id');
            if ($tahun) {
                $canonicalUrl = route('monitoring.rencanaawal.urusan.tahun', [
                    'id' => $id,
                    'urusanId' => $urusanId,
                    'tahun' => $tahunAktif->tahun,
                ]);
            } else {
                $canonicalUrl = route('monitoring.rencanaawal.urusan', [
                    'id' => $id,
                    'urusanId' => $urusanId,
                ]);
            }
            if ($periodeId) {
                $canonicalUrl .= '?periode_id=' . $periodeId;
            }
            return redirect()->to($canonicalUrl);
        }

        // Jika masih ada origin_user_id di query, redirect ke URL tanpa parameter tersebut untuk merapikan URL
        if ($request->query->has('origin_user_id')) {
            $query = $request->query();
            unset($query['origin_user_id']);
        ...
        // Get all available urusan for this SKPD - hanya yang memiliki data (non-null id_urusan)
        $availableUrusanIdsCollection = collect($availableUrusanIds);
        // Pastikan urusan yang sedang dipilih (dari tugas yang diklik) ikut tampil di daftar
        if (!empty($urusanId)) {
            $availableUrusanIdsCollection->push($urusanId);
        }

        // Fallback: Jika kosong, tampilkan semua urusan (edge case) agar UI tetap jalan
        if ($availableUrusanIdsCollection->isEmpty()) {
            $allUrusans = KodeNomenklatur::where('jenis_nomenklatur', 0)->get();
        } else {
            $allUrusans = KodeNomenklatur::where('jenis_nomenklatur', 0)
                ->whereIn('id', $availableUrusanIdsCollection->unique()->values())
                ->get();
        }

        \Log::info('RencanaAwal available urusan FINAL', [
            'available_urusan_ids' => $availableUrusanIdsCollection->unique()->values()->toArray(),
        ...
        // PERBAIKAN: Rapikan URL. Hilangkan origin_user_id dari URL.
        // Derive breadcrumb user id dari session atau referer; fallback ke pencarian otomatis.
        $breadcrumbUserId = null;
        if ($request->filled('origin_user_id')) {
            // Transitional support: jika masih ada param lama, simpan ke session lalu gunakan
            $breadcrumbUserId = (int) $request->input('origin_user_id');
            session(['breadcrumb_user_id' => $breadcrumbUserId]);
        } elseif (session()->has('breadcrumb_user_id')) {
            $breadcrumbUserId = (int) session('breadcrumb_user_id');
        } else {
            $referer = $request->headers->get('referer');
            if ($referer && preg_match('#/rencana-awal/(\d+)(?:/|\?|$)#', $referer, $m)) {
                $breadcrumbUserId = (int) $m[1];
                session(['breadcrumb_user_id' => $breadcrumbUserId]);
            } else {
        ...
        // Get all periods with complete data
        $semuaPeriodeAktif = Periode::with(['tahap', 'tahun'])->orderBy('id', 'desc')->get();
        ...
        return Inertia::render('Monitoring/RencanaAwal', [
            ...
            'semuaPeriodeAktif' => $semuaPeriodeAktif, // Kirim SEMUA periode dengan status
    ...
    {
        // Kumpulkan seluruh id_urusan dari semua tugas (jenis >= 1) yang memiliki parent urusan
        $ids = $skpdTugas
            ->filter(function($item) {
                return $item->kodeNomenklatur
                    && $item->kodeNomenklatur->details->first()
                    && !is_null($item->kodeNomenklatur->details->first()->id_urusan);
            })
    ...
    // create() removed: unused and previously referenced removed view 'Monitoring/Create'
    ...
    public function saveMonitoringData(Request $request)
    {
        ...
        try {
            // PERBAIKAN: Tambahkan validasi periode Rencana Awal yang sama seperti saveTarget
            if ($validated['periode_id']) {
                $selectedPeriode = Periode::with('tahap', 'tahun')->find($validated['periode_id']);
                
                if (!$selectedPeriode) {
                    return response()->json(['error' => 'Periode yang dipilih tidak valid.'], 400);
                }
                
                // Validasi harus periode Rencana Awal
                $isRencanaAwalPeriode = stripos($selectedPeriode->tahap->tahap ?? '', 'rencana') !== false;
                if (!$isRencanaAwalPeriode) {
                    return response()->json(['error' => 'Anda berada di halaman Rencana Awal, namun periode yang dipilih bukan periode Rencana Awal.'], 400);
                }
                
                if ($selectedPeriode->status != 1) {
                    $statusText = $selectedPeriode->status == 0 ? 'ditutup' : 'selesai';
                    return response()->json(['error' => "Periode {$selectedPeriode->tahap->tahap} telah {$statusText} oleh admin. Data tidak dapat diubah pada periode ini."], 403);
                }
            }
    ...
    public function saveTarget(Request $request)
    {
        ...
        try {
            // PERBAIKAN: Validasi periode yang spesifik dan lebih ketat untuk Rencana Awal
            
            // 1. Cek apakah periode_id yang dikirim valid dan aktif
            if ($validated['periode_id']) {
                $selectedPeriode = Periode::with('tahap', 'tahun')->find($validated['periode_id']);
                ...
                // PERBAIKAN: Validasi khusus untuk Rencana Awal - harus periode dengan tahap "Rencana"
                $isRencanaAwalPeriode = stripos($selectedPeriode->tahap->tahap ?? '', 'rencana') !== false;
                if (!$isRencanaAwalPeriode) {
                    return redirect()->route('monitoring.rencanaawal', ['id' => $validated['skpd_tugas_id']])
                        ->with('error', 'Anda berada di halaman Rencana Awal, namun periode yang dipilih bukan periode Rencana Awal.');
                }
                
                if ($selectedPeriode->status != 1) {
                    $statusText = $selectedPeriode->status == 0 ? 'ditutup' : 'selesai';
                    return redirect()->route('monitoring.rencanaawal', ['id' => $validated['skpd_tugas_id']])
                        ->with('error', "Periode {$selectedPeriode->tahap->tahap} telah {$statusText} oleh admin. Data tidak dapat diubah pada periode ini.");
                }
                
                Log::info('Rencana Awal periode validation passed', [
                    'periode_id' => $selectedPeriode->id,
                    'periode_status' => $selectedPeriode->status,
                    'tahap' => $selectedPeriode->tahap->tahap ?? 'Unknown',
                    'tahun' => $selectedPeriode->tahun->tahun ?? 'Unknown'
                ]);
            } else {
                // 2. Jika tidak ada periode_id, cari periode Rencana Awal yang aktif
                $rencanaAwalPeriode = Periode::with('tahap')
                ...
                if (!$rencanaAwalPeriode) {
                    return redirect()->route('monitoring.rencanaawal', ['id' => $validated['skpd_tugas_id']])
                        ->with('error', 'Periode Rencana Awal tidak aktif. Data tidak dapat disimpan saat ini.');
                }
            }
            ...
            foreach ($validated['targets'] as $target) {
                ...
                $periode = \App\Models\Periode::whereHas('tahap', function($query) use ($triwulanName) {
                        $query->where('tahap', 'like', '%' . $triwulanName . '%');
                    })
                    ->whereHas('tahun', function($query) use ($correctTahun) {
                        $query->where('tahun', $correctTahun);
                    })
                    ->first();
    ...
    /**
        // PERBAIKAN: Gunakan origin_user_id dari request jika tersedia agar breadcrumb kembali ke user yang sama
        $breadcrumbUserId = $request->input('origin_user_id')
            ? (int) $request->input('origin_user_id')
            : $this->getBreadcrumbUserId($tugas);
     */
