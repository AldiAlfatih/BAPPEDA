...
const breadcrumbs = computed<BreadcrumbItem[]>(() => [
  { title: 'Monitoring Triwulan 3', href: '/triwulan/3' },
  { title: `Monitoring Detail ${props.user.nama_skpd}`, href: route('triwulan.show', { tid: 3, id: props.user.id }) },
...
const saveData = (id: number) => {
  ...
  const formData = {
    ...
    nama_pptk: item.pptk
          ...
          console.log('Sending data using Inertia router to:', route('triwulan.save-realisasi', { tid: 3 }));
  ...
  router.post(route('triwulan.save-realisasi', { tid: 3 }), formData, {
    preserveState: true,
    preserveScroll: true,
    onStart: () => {
...
const fetchAkumulasiKinerjaTahunan = async (skpdTugasId, tahun = null) => {
  try {
    const tahunParam = tahun || new Date().getFullYear();
            const response = await fetch(`/triwulan/akumulasi-kinerja/${skpdTugasId}/${tahunParam}`);
...
<template>
    <Head title="Detail Monitoring Triwulan 3" />
 ...
 <div class="flex h-full flex-1 flex-col gap-4 p-4 bg-gray-100 dark:bg-gray-800">
            ...
            <div class="rounded-lg bg-white p-6 shadow-lg border border-gray-100">
                <div class="flex items-center mb-6">
                    ...
                    <h2 class="text-2xl font-bold text-gray-600">Triwulan 3</h2>
                </div>
